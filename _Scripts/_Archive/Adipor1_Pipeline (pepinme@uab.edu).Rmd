...
```{r Pathway_merge}
library(openxlsx)
data_heart<-read.xlsx("../1_Input/Pathway.Overlap_3Tissues.xlsx", sheet = "Cardiac")
data_kidney<-read.xlsx("../1_Input/Pathway.Overlap_3Tissues.xlsx", sheet = "Kidney")
data_liver<-read.xlsx("../1_Input/Pathway.Overlap_3Tissues.xlsx", sheet = "Liver")
...
library(corrplot)
library(pheatmap)
cor<-data.matrix(data.pca)
cor.m<-rcorr(cor)
cor.r<-cor(cor)
cor.r<-data.matrix(cor.r)


cor.r<-as.numeric(as.character(cor.r))
paletteLength <- 100
myColor <- colorRampPalette(c("dodgerblue4", "white", "brown4"))(paletteLength)
r.mat<-cor.mtest(cor.r)$r
heatmap(cor.r)
corrplot(cor.r, 
         order="AOE",
         type="lower",
...
##Dendrogram
library(dendextend)
dd <- dist(scale(t(transformed_test)), method = "euclidean")
hc <- hclust(dd, method = "ward.D2")
hcd <- as.dendrogram(hc)
#Create Dendrogram
hcd %>% set("branches_k_color", 
             value = c("gold2", "dodgerblue4"), k = 2) %>% 
